...
{
	return static_cast<DIR*>(Lua::UT(L, 1));
}

static struct dirent * E (lua_State * L)
{
	return static_cast<struct dirent*>(Lua::UT(L, 1));
}

static int OpenDir (lua_State * L)
{
	DIR * dir = opendir(Lua::S(L, 1));

	if (dir != 0)
	{
		Lua::PushUserType(L, dir, "DIR");
...
static int ReadDir (lua_State * L)
{
	...
	if (entry != 0)
	{
		Lua::PushUserType(L, entry, "struct dirent");
...
static int SeekDir (lua_State * L)
{
	...
	if (dir != 0 && lua_isnumber(L, 2)) seekdir(dir, Lua::LI(L, 2));
...
void luaopen_dirent (lua_State * L)
{
	...
	Lua::UserType_Reg UT[2] = { 0 };

	UT[0].mName = "d_namlen";
	UT[0].mOffset = offsetof(struct dirent, d_namlen);
	UT[0].mType = Lua::UserType_Reg::eUShort;

	UT[1].mName = "d_name";
	UT[1].mOffset = offsetof(struct dirent, d_name);
	UT[1].mType = Lua::UserType_Reg::eString;

	Lua::RegisterUserType(L, "DIR", 0, 0, 0, "");
	Lua::RegisterUserType(L, "struct dirent", UT, sizeof(UT) / sizeof(UT[0]), 0, "");
	...
	luaL_register(L, "dirent", DirentFuncs);
}
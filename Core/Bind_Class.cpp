...
/// @note ...: Arguments to routine
/// @note Return: Results of call
static int InvokeIf_ (lua_State * L)
{
	if (lua_isnoneornil(L, 1)) luaL_error(L, "I:InvokeIf_: nil instance");
	if (lua_isnoneornil(L, 2)) luaL_error(L, "I:InvokeIf_: nil method");
	...
	if (lua_pcall(L, count + 1, LUA_MULTRET, 0) != 0) luaL_error(L, "I:InvokeIf_: member not callable");// results
...
static int New (lua_State * L)
{
	...
	lua_rawseti(L, -2, int(lua_objlen(L, -2) + 1));	// _N[new top] = I -> new, ..., I, _D, _D[type], _N
	...
	lua_rawseti(L, 2, int(lua_objlen(L, 2)));	// _N[old top] = nil -> I, _N
	lua_pop(L, 1);	// I
...
static int SCons (lua_State * L)
{
	...
	lua_rawgeti(L, -1, int(lua_objlen(L, -1)));	// base, ..., _N, I
